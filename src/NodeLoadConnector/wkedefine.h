...
__inline static
HGLOBAL wkeOpenResource(LPVOID & lpData, DWORD & dwSize, LPCTSTR lpName, LPCTSTR lpType, HMODULE hModule = ::GetModuleHandle(NULL))
{
	BOOL bResult = FALSE;
	// handle/ptr. to res. info. in hSource 
	HRSRC hRsrcRes = NULL;
...
__inline static
BOOL wkeParseResrc(LPCTSTR ptszFileName, UINT uResourceID, LPCTSTR ptszTypeName, HMODULE hModule = ::GetModuleHandle(NULL))
{
	DWORD dwSize = 0;
	HANDLE hFile = NULL;
	BOOL bResult = FALSE;
	...
	hGlobal = wkeOpenResource(lpData, dwSize, MAKEINTRESOURCE(uResourceID), ptszTypeName, hModule);
...
#endif
//	ANSI to Unicode
__inline static std::wstring wkeANSIToUnicode(const std::string str)
{
	int len = 0;
	len = str.length();
...
//Unicode to ANSI
__inline static std::string wkeUnicodeToANSI(const std::wstring str)
{
	char* pElementText;
	int iTextLen;
...
//UTF - 8 to Unicode
__inline static std::wstring wkeUTF8ToUnicode(const std::string str)
{
	int len = 0;
	len = str.length();
...
//Unicode to UTF - 8
__inline static std::string wkeUnicodeToUTF8(const std::wstring str)
{
	char*   pElementText;
	int iTextLen;
...
__inline static std::string wkeTToA(tstring tsT)
{
	std::string str = "";
	...
	str = tsT;
#else
	str = wkeUnicodeToANSI(tsT);
#endif

	return str;
}

__inline static std::wstring wkeTToW(tstring tsT)
{
	std::wstring wstr = L"";

#if !defined(UNICODE) && !defined(_UNICODE)
	wstr = wkeANSIToUnicode(tsT);
#else
	wstr = tsT;
#endif
...
__inline static tstring wkeAToT(std::string str)
{
	tstring ts = _T("");
	...
	ts = str;
#else
	ts = wkeANSIToUnicode(str);
#endif

	return ts;
}

__inline static tstring wkeWToT(std::wstring wstr)
{
	tstring ts = _T("");

#if !defined(UNICODE) && !defined(_UNICODE)
	ts = wkeUnicodeToANSI(wstr);
#else
	ts = wstr;
#endif
...
inline void wkeStartup()
{
	//_TCHAR tzTempFile[MAX_PATH] = { 0 };
	//::GetTempPath(sizeof(tzTempFile) / sizeof(*tzTempFile), tzTempFile);
	//lstrcat(tzTempFile, _T("DLL.DLL"));
	//if (sizeof(void *) > sizeof(unsigned long))
	//{
	//	wkeParseResrc(tzTempFile, IDR_IDCNODEDLL64, MAKEINTRESOURCE(IDR_IDCNODEDLL64));
	//}
	//else
	//{
	//	wkeParseResrc(tzTempFile, IDR_IDCNODEDLL86, MAKEINTRESOURCE(IDR_IDCNODEDLL86));
	//}
}
inline void wkeCleanup()
{
	_TCHAR tzTempFile[MAX_PATH] = { 0 };
	::GetTempPath(sizeof(tzTempFile) / sizeof(*tzTempFile), tzTempFile);
	lstrcat(tzTempFile, _T("DLL.DLL"));
	::_tunlink(tzTempFile);
	::DeleteFile(tzTempFile);
}
inline void wkeResourceInitializeEx(const wkeSettings* settings)
{
	_TCHAR tzTempFile[MAX_PATH] = { 0 };
	//::GetTempPath(sizeof(tzTempFile) / sizeof(*tzTempFile), tzTempFile);
	//lstrcat(tzTempFile, _T("DLL.DLL"));
	if (sizeof(void *) > sizeof(unsigned long))
	{
		_stprintf(tzTempFile, _T("Node64.DLL\0"));
	}
	else
	{
		_stprintf(tzTempFile, _T("Node86.DLL\0"));
	}
	HMODULE hMod = LoadLibrary(tzTempFile);
...
inline void wkeResourceInit()
{
	wkeResourceInitializeEx(((const wkeSettings*)0));
}

inline void wkeResourceInitialize()
{
	wkeResourceInitializeEx(((const wkeSettings*)0));
}
